...
from agents.att_iddpg.buffer import Experience, ReplayMemory
from agents.att_iddpg.networks import (
    ActorNetworkVariableNeiWRadar,
    CriticNetworkVariableNeiWRadar,
)


class ATTIDDPGTrainer(BaseTrainer):
    def __init__(self, config):
        ...
        self.tcpa_bias_scale = float(config["train"].get("tcpa_bias_scale", 4.0))
    ...
    def _require_supported_configuration(self):
        if self.flags.get("full_observable_critic"):
            raise NotImplementedError("att-iddpg only supports non-full-observable critic mode.")
        if self.flags.get("use_gru"):
            raise NotImplementedError("att-iddpg does not support GRU mode yet.")
        if self.flags.get("own_obs_only"):
            raise NotImplementedError("att-iddpg does not support own_obs_only mode.")

    def _extract_dims(self, obs):
        if len(obs) < 4:
            raise ValueError("att-iddpg expects four observation portions: own obs, fixed neighbors, radar, variable neighbors.")
        own_dim = len(obs[0][0])
    ...
    def _ensure_models_initialized(self, obs):
        ...
        self.actor = ActorNetworkVariableNeiWRadar(
            ...
            self.action_dim,
            tcpa_bias_scale=self.tcpa_bias_scale,
    ...
    def save(self, path, episode=None, step=None, stop_mode=None):
        ...
        actor_path = os.path.join(path, "att_iddpg_actor.pt")
        critic_path = os.path.join(path, "att_iddpg_critic.pt")
        torch.save(self.actor.state_dict(), actor_path)
        torch.save(self.critic.state_dict(), critic_path)

        if episode is not None:
            torch.save(self.actor.state_dict(), os.path.join(path, "att_iddpg_actor_ep{}.pt".format(int(episode))))
            torch.save(self.critic.state_dict(), os.path.join(path, "att_iddpg_critic_ep{}.pt".format(int(episode))))
        if step is not None:
            torch.save(self.actor.state_dict(), os.path.join(path, "att_iddpg_actor_step{}.pt".format(int(step))))
            torch.save(self.critic.state_dict(), os.path.join(path, "att_iddpg_critic_step{}.pt".format(int(step))))

    def _load_state(self, path, checkpoint_tag=None):
        actor_name = "att_iddpg_actor.pt"
        critic_name = "att_iddpg_critic.pt"
        if checkpoint_tag:
            actor_name = f"att_iddpg_actor_{checkpoint_tag}.pt"
            critic_name = f"att_iddpg_critic_{checkpoint_tag}.pt"
