...
from agents.pcb_biased_att_iddpg.buffer import Experience, ReplayMemory
from agents.pcb_biased_att_iddpg.networks import (
    ActorNetworkVariableNeiWRadar,
    CriticNetworkVariableNeiWRadar,
)


class PCBBiasedATTIDDPGTrainer(BaseTrainer):
    ...
    def _require_supported_configuration(self):
        if self.flags.get("full_observable_critic"):
            raise NotImplementedError("pcb_biased_att_iddpg only supports non-full-observable critic mode.")
        if self.flags.get("use_gru"):
            raise NotImplementedError("pcb_biased_att_iddpg does not support GRU mode yet.")
        if self.flags.get("own_obs_only"):
            raise NotImplementedError("pcb_biased_att_iddpg does not support own_obs_only mode.")

    def _extract_dims(self, obs):
        if len(obs) < 4:
            raise ValueError("pcb_biased_att_iddpg expects four observation portions: own obs, fixed neighbors, radar, variable neighbors.")
        own_dim = len(obs[0][0])
    ...
    def save(self, path, episode=None, step=None, stop_mode=None):
        ...
        actor_path = os.path.join(path, "pcb_biased_att_iddpg_actor.pt")
        critic_path = os.path.join(path, "pcb_biased_att_iddpg_critic.pt")
        torch.save(self.actor.state_dict(), actor_path)
        torch.save(self.critic.state_dict(), critic_path)

        if episode is not None:
            torch.save(self.actor.state_dict(), os.path.join(path, "pcb_biased_att_iddpg_actor_ep{}.pt".format(int(episode))))
            torch.save(self.critic.state_dict(), os.path.join(path, "pcb_biased_att_iddpg_critic_ep{}.pt".format(int(episode))))
        if step is not None:
            torch.save(self.actor.state_dict(), os.path.join(path, "pcb_biased_att_iddpg_actor_step{}.pt".format(int(step))))
            torch.save(self.critic.state_dict(), os.path.join(path, "pcb_biased_att_iddpg_critic_step{}.pt".format(int(step))))

    def _load_state(self, path, checkpoint_tag=None):
        actor_name = "pcb_biased_att_iddpg_actor.pt"
        critic_name = "pcb_biased_att_iddpg_critic.pt"
        if checkpoint_tag:
            actor_name = f"pcb_biased_att_iddpg_actor_{checkpoint_tag}.pt"
            critic_name = f"pcb_biased_att_iddpg_critic_{checkpoint_tag}.pt"
